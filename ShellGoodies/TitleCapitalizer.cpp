...
CCapitalizeOptions::CCapitalizeOptions( void )
	...
	, m_alwaysPreserveWords( _T("live,bonus,mix,remix,cover,version,so,st,nd,rd,CD,BBC,TV"), cap::Preserve )
...
std::tstring CTitleCapitalizer::GetCapitalized( const std::tstring& text ) const
{
	...
	str::CTokenIterator<pred::TStrEqualsIgnoreCase> it( text );

	if ( m_pOptions->m_skipNumericPrefix )
		if ( !it.AtEnd() && _istdigit( *it ) )
			while ( !it.AtEnd() && pred::IsNumericPrefix( *it ) || '.' == *it )
			{
				oss << *it;
				++it;
			}
