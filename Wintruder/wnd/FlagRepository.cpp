...
#define FLAG_NAME( value, mask, name ) { value, mask, CFlagInfo::Editable, _T(#name) }
...
namespace style
{
	...
	namespace specific
	{
		...
		CFlagInfo buttonFlags[] =
		{
			...
			FLAG( BS_LEFT, BS_LEFT | BS_RIGHT | BS_CENTER ),
			FLAG( BS_RIGHT, BS_LEFT | BS_RIGHT | BS_CENTER ),
			FLAG( BS_CENTER, BS_LEFT | BS_RIGHT | BS_CENTER ),
			GROUP_SEPARATOR( _T("Vertical Align") ),
			FLAG( BS_TOP, BS_TOP | BS_BOTTOM | BS_VCENTER ),
			FLAG( BS_BOTTOM, BS_TOP | BS_BOTTOM | BS_VCENTER ),
			FLAG( BS_VCENTER, BS_TOP | BS_BOTTOM | BS_VCENTER ),
			GROUP_SEPARATOR( _T("More") ),
			FLAG( BS_PUSHLIKE, 0 ),
			FLAG( BS_MULTILINE, 0 ),
			FLAG( BS_NOTIFY, 0 ),
...
{
	static stdext::hash_map< std::tstring, CFlagStore* > classToStoreMap;
	...
	stdext::hash_map< std::tstring, CFlagStore* >::const_iterator itFound = classToStoreMap.find( str::MakeUpper( wndClass ) );
