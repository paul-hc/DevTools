...
void CEndiannessTests::TestSwapBytesValues( void )
{
	...
	{	// no byte swapping
		const char value = 'A';
		char swapped = GetBytesSwapped<LittleEndian, BigEndian>()( value );
		ASSERT_EQUAL( swapped, value );

		char swappedBack = GetBytesSwapped<BigEndian, LittleEndian>()( swapped );
...

	{
		...
		wchar_t swapped = GetBytesSwapped<LittleEndian, BigEndian>()( value );
		ASSERT_EQUAL( L'\x4100', swapped );

		wchar_t swappedBack = GetBytesSwapped<BigEndian, LittleEndian>()( swapped );
		ASSERT_EQUAL( value, swappedBack );
	}

	{
		const int value = 0x2211;
		int swapped = GetBytesSwapped<LittleEndian, BigEndian>()( value );
		ASSERT_EQUAL( 0x11220000, swapped );

		int swappedBack = GetBytesSwapped<BigEndian, LittleEndian>()( swapped );
		ASSERT_EQUAL( value, swappedBack );
	}

	{
		const long long value = 0x1234ll;
		long long swapped = GetBytesSwapped<LittleEndian, BigEndian>()( value );
		ASSERT_EQUAL( 0x3412000000000000ll, swapped );

		long long swappedBack = GetBytesSwapped<BigEndian, LittleEndian>()( swapped );
		ASSERT_EQUAL( value, swappedBack );
	}

	{
		const float value = 1.23f;
		float swapped = GetBytesSwapped<LittleEndian, BigEndian>()( value );
		ASSERT( swapped != value );

		float swappedBack = GetBytesSwapped<BigEndian, LittleEndian>()( swapped );
		ASSERT_EQUAL( value, swappedBack );
	}

	{
		const double value = 7.43;
		double swapped = GetBytesSwapped<LittleEndian, BigEndian>()( value );
		ASSERT( swapped != value );

		double swappedBack = GetBytesSwapped<BigEndian, LittleEndian>()( swapped );
		ASSERT_EQUAL( value, swappedBack );
	}
}
...
void CEndiannessTests::TestSwapBytesString( void )
{
	...
	// wchar_t (UTF16)
	{
		...
		std::for_each( swappedText.begin(), swappedText.end(), func::SwapBytes<LittleEndian, BigEndian>() );
		ASSERT_EQUAL( L"\x4100\x4200\x4300\x4400", swappedText );

		endian::SwapBytes<BigEndian, LittleEndian>( swappedText );		// swap back
	...
	// char32_t (UTF32)
	{
		...
		endian::SwapBytes<LittleEndian, BigEndian>( swappedText );		// swap back
		...
		std::for_each( swappedText.begin(), swappedText.end(), func::SwapBytes<BigEndian, LittleEndian>() );
		ASSERT_EQUAL( value, swappedText );
	}
}
...
void CEndiannessTests::TestSwapBytesContainer( void )
{
	...
	std::for_each( swappedWords.begin(), swappedWords.end(), func::SwapBytes<LittleEndian, BigEndian>() );
	ASSERT_EQUAL( 0xBBAA, swappedWords[ 0 ] );
	ASSERT_EQUAL( 0xDDCC, swappedWords[ 1 ] );
	ASSERT_EQUAL( 0xFFEE, swappedWords[ 2 ] );

	std::for_each( swappedWords.begin(), swappedWords.end(), func::SwapBytes<BigEndian, LittleEndian>() );
...
void CEndiannessTests::TestSwapBytesSameEndianness( void )
{
	...
	wchar_t wc = GetBytesSwapped<LittleEndian, LittleEndian>()( L'A' );
	ASSERT_EQUAL( L'A', wc );

	UINT ui = GetBytesSwapped<LittleEndian, LittleEndian>()( 0x2211u );
	ASSERT_EQUAL( 0x2211u, ui );

	ULONGLONG ui64 = GetBytesSwapped<BigEndian, BigEndian>()( (ULONGLONG)0x2211ull );
	ASSERT_EQUAL( 0x2211ull, ui64 );

	float f = GetBytesSwapped<BigEndian, BigEndian>()( 1.23f );
	ASSERT_EQUAL( 1.23f, f );

	double d = GetBytesSwapped<BigEndian, BigEndian>()( 7.43 );
	ASSERT_EQUAL( 7.43, d );
	...
	std::wstring swappedText = value;
	std::for_each( swappedText.begin(), swappedText.end(), func::SwapBytes<LittleEndian, LittleEndian>() );
	ASSERT_EQUAL( value, swappedText );
}
