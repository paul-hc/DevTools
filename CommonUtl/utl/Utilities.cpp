...
namespace ui
{
	...
	CRect StretchToFit( const CRect& destBoundsRect, const CSize& srcSize, CSize spacingSize /*= CSize( 0, 0 )*/, int alignment /*= H_AlignCenter | V_AlignCenter*/ )
	{
		...
		CSize destSize = StretchToFit( destBoundsRect.Size() - spacingSize, srcSize - spacingSize );
	...
	CRect& AlignRect( CRect& rDest, const CRect& anchor, int alignment, bool limitDest /*= false*/ )
	{
		...
		switch ( alignment & HorizontalMask )
		{
			case H_AlignLeft:
				...
				break;
			case H_AlignCenter:
				offset.cx = anchor.left - rDest.left + ( anchor.Width() - rDest.Width() ) / 2;
				break;
			case H_AlignRight:
		...
		switch ( alignment & VerticalMask )
		{
			case V_AlignTop:
				...
				break;
			case V_AlignCenter:
				offset.cy = anchor.top - rDest.top + ( anchor.Height() - rDest.Height() ) / 2;
				break;
			case V_AlignBottom:
	...
	CRect& CenterRect( CRect& rDest, const CRect& anchor, bool horiz /*= true*/, bool vert /*= true*/, bool limitDest /*= false*/,
					   const CSize& offset /*= CSize( 0, 0 )*/ )
	{
		CPoint delta = anchor.TopLeft() - rDest.TopLeft() + ScaleSize( anchor.Size() - rDest.Size() + offset, 1, 2 );
...
namespace ui
{
	...
	{
		return AfxMessageBox( CString( e.what() ), mbFlags );
	}
	...
	{
		static const pred::Comparator< pred::CompareCode > compareCodeAsc;
		if ( NULL == pComparator )
