...
namespace shell
{
	...
	public:
		...
		bool IsValid( void ) const { return m_pDragSourceHelper != NULL; }

		bool SetFromWindow( HWND hWnd, const POINT* pOrigin = NULL );		// window must handle DI_GETDRAGIMAGE message (built-in by CListCtrl, CTreeCtrl)

		// shell helper takes ownership of the bitmap;  transpColor: CLR_NONE for no transparent color, CLR_DEFAULT for COLOR_WINDOW
		bool SetFromBitmap( HBITMAP hBitmap, COLORREF transpColor, const POINT* pOrigin = NULL );
