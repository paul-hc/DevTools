...
CAlbumNavigator::CAlbumNavigator( const CAlbumImageView* pAlbumView )
	...
	, m_imageInfo( pAlbumView->GetImage() )
...
bool CAlbumNavigator::CanNavigate( nav::Navigate navigate, bool multiFrame /*= true*/ ) const
{
	nav::TIndexFramePosPair currentInfo = GetCurrentInfo();
	nav::TIndexFramePosPair newNavigInfo = GetNavigateInfo( navigate, multiFrame );

	return newNavigInfo != currentInfo;
}

nav::TIndexFramePosPair CAlbumNavigator::GetNavigateInfo( nav::Navigate navigate, bool multiFrame /*= true*/ ) const
{
	...
	if ( m_navigCount != 0 )
		switch ( navigate )
		{
			...
			case nav::Next:
				...
				if ( multiFrame && m_imageInfo.IsMultiFrameImage() )
				{
					utl::AdvancePos( navigInfo.second, m_imageInfo.m_frameCount, false, nav::Next == navigate );

					if ( navigInfo.second != m_imageInfo.m_framePos )		// current frame changed?
						return navigInfo;			// we're done navigating at frame level
				}

				utl::AdvancePos( navigInfo.first, m_navigCount, m_pSlideData->m_wrapMode, nav::Next == navigate );
				navigInfo.second = 0;

				if ( navigInfo.first != m_navigPos )						// current index changed?
					if ( multiFrame && nav::Previous == navigate )			// navigated to previous image?
...
// CAlbumNavigator::CImageInfo implementation

CAlbumNavigator::CImageInfo::CImageInfo( const CWicImage* pImage )
{
	if ( pImage != NULL )
	{
