...
void CAlbumSettingsDialog::SetupFoundListView( void )
{
	...
	size_t count = m_fileList.GetFileAttrCount();
	{
		...
		for ( UINT i = 0; i != count; ++i )
		{
			...
			m_foundFilesListCtrl.SetSubItemText( i, Folder, pFileAttr->GetPath().GetParentPath() );
...
BEGIN_MESSAGE_MAP( CAlbumSettingsDialog, CLayoutDialog )
	...
	ON_CONTROL( CReportListControl::LVN_ItemsReorder, IDC_FOUND_FILES_LISTVIEW, OnLVnItemsReorder_FoundFiles )
